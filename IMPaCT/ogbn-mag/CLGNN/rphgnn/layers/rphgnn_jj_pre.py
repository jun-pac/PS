...
def rphgnn_propagate_and_collect(g, k, inner_k, alpha, target_node_type, use_input_features, squash_strategy, train_label_feat, norm, squash_even_odd, collect_even_odd, y, paper_year, squash_self=False, target_feat_random_project_size=None, add_self_group=False):
    ...
    with torch.no_grad():
        ...
        with g.local_scope():
            ...
            for k_ in range(k):
                ...
                print(f"alpha : {alpha}")
                # for ntype in g.ntypes:
                #     print(f"ntype: {ntype}, g.nodes[ntype].data['feat'].shape : {g.nodes[ntype].data['feat'].shape}, input_x_dict[ntype].shape : {input_x_dict[ntype].shape}")
                #     g.nodes[ntype].data["feat"] = g.nodes[ntype].data["feat"] * (1 - alpha) + input_x_dict[ntype] * alpha

                x_clone=torch.clone(g.nodes[target_node_type].data["feat"])
                print(f"K : {k_}, target_node_type : {target_node_type}, x_clone.shape : {x_clone.shape}",flush=True)

                t1=time.time()
                shapes=list(x_clone.shape)
                print(f"shapes : {shapes}")
                train_mean=torch.zeros((8,349,shapes[1]))
                train_cnt=torch.zeros((8,349))
                train_time_mean=torch.zeros((10,shapes[1]))
                ...
                for u in range(shapes[0]):
                    if(paper_year[u]==2019):
                        ...
                        train_mean[paper_year[u]-2010][y[u]]+=x_clone[u]
                ...
                for u in range(shapes[0]):
                    if(paper_year[u]==2019):
                        test_var+=LA.norm(x_clone[u]-test_mean)**2
                    elif(paper_year[u]<=2017):
                        t=paper_year[u]-2010
                        msq[t]+=LA.norm(train_mean[t][y[u]]-train_time_mean[t])**2
                        rsq[t]+=LA.norm(x_clone[u]-train_mean[t][y[u]])**2
                ...
                for u in range(shapes[0]):
                    if(paper_year[u]<=2017):
                        t=paper_year[u]-2010
                        x_clone[u]=alpha[t]*x_clone[u]+(1-alpha[t])*train_mean[t][y[u]]
                g.nodes[target_node_type].data["feat"]=x_clone                
                print(f"K({k_})th feature normalization time : {time.time()-t1:.4f}, g.nodes[target_node_type].data['feat'].shape : {g.nodes[target_node_type].data['feat'].shape}",flush=True)
...
def rphgnn_propagate_and_collect_for_label(g, k, inner_k, alpha, target_node_type, use_input_features, squash_strategy, train_label_feat, norm, squash_even_odd, collect_even_odd, y, paper_year, squash_self=False, target_feat_random_project_size=None, add_self_group=False):
...
def rphgnn_propagate_and_collect_label(hetero_graph, target_node_type, y, train_label_feat, paper_year):

    label_target_h_list_list, _ = rphgnn_propagate_and_collect_for_label(hetero_graph, 
                1, 
                2, 
                0.0,
