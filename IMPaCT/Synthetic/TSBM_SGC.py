...
def train_base(g, f_log):
    ...
    model = SGConv(features.shape[1], 16, args.num_label, k=2, cached=True, bias=True).to(device)
...
def train_JJ(g, f_log, times):
    ...
    labels = g.ndata["label"]
    masks = g.ndata["train_mask"].type(torch.bool), g.ndata["val_mask"].type(torch.bool), g.ndata["test_mask"].type(torch.bool)
    model = SGConv_JJ(labels, times, args.num_time, args.num_label, args.split, features.shape[1], 16,args.num_label, k=2, cached=True, bias=True).to(device)
    train(g, features, labels, masks, model, f_log)
    acc = evaluate(g, features, labels, masks[2], model)
    # print(f"{acc:.4f}", end=' ', flush=True)
    f_log.write(f"{acc:.4f}\n")
...
def train_PNY(g, f_log, relative_conn, times):
    ...
    labels = g.ndata["label"]
    masks = g.ndata["train_mask"].type(torch.bool), g.ndata["val_mask"].type(torch.bool), g.ndata["test_mask"].type(torch.bool)
    model = SGConv_PNY(labels, times, args.num_time, args.num_label, args.split, relative_conn, features.shape[1], 16,args.num_label, k=2, cached=True, bias=True).to(device)
    train(g, features, labels, masks, model, f_log)
    acc = evaluate(g, features, labels, masks[2], model)
    # print(f"{acc:.4f}", end=' ', flush=True)
    f_log.write(f"{acc:.4f}\n")
...
if __name__ == "__main__":
    ...
    for i in range(args.repeat):
        ...
        if args.L==1: # When assumption 4 does not hold
...
while sleep 1; do python TSBM_SGC.py --decay 0.3 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --decay 0.4 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --decay 0.5 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --decay 0.6 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --decay 0.7 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --decay 0.8 --epoch 200; done

while sleep 1; do python TSBM_SGC.py --decay 0.35 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --decay 0.45 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --decay 0.55 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --decay 0.65 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --decay 0.75 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --decay 0.85 --epoch 200; done


# When assumption 4 does not hold
while sleep 1; do python TSBM_SGC.py --L 1 --epoch 200; done
'''