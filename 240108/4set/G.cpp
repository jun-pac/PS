...
void Solve(){
    ...
    rng(i,0,n-1){
        ...
        order[2*i]={pos[i].fi,i,0};
        order[2*i+1]={pos[i].se,i,1};
    }
    ...
    rng(i,0,2*n-1){
        ...
        if(order[i][2]==1){
            res+=seg.find_seg(1,0,2*n-1,pos[order[i][1]].fi,i);
            seg.update_seg(1,0,2*n-1,pos[order[i][1]].fi,1);
        }
    }
    cout<<res<<'\n';
}
