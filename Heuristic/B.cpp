...
inline bool check(int a, int b, char ad, char bd){
    if(ad!='.') if(!check1(a,ad)){
        return false;
    }
    if(bd!='.') if(!check1(b,bd)){
        return false;
    }
    return true;
}
...
ll swap_is_better(int aidx, int bidx){
    ...
    //cout<<"New - Original ? "<<new_res<<' '<<ori_res<<' '<<new_res-ori_res<<'\n';
    swap(datas[ax][ay], datas[bx][by]);
...
void DFS(int aidx){
    ...
    for(auto anext : edges[aidx]){
        ...
        // if(!check1(anext.fi, rev(anext.se))){
        //     cout<<"why2? "<<anext.fi/n<<' '<<anext.fi%n<<' '<<anext.se<<'\n';
        //     return;
        // }
        //check(anext.fi,bidx,rev(anext.se),e);
        //if(!check(anext.fi,bidx,anext.se,e)) cout<<"False "<<aidx<<' '<<bidx<<' '<<anext.se<<' '<<e<<'\n';
...
void Solve(){
    ...
    // cout<<"raw_edges"<<'\n';
    // rng(i,0,11){
    //     for(auto ttt : raw_edges[i]){
    //         cout<<i<<' '<<ttt.fi<<' '<<ttt.se<<'\n';
    //     }
    // }
    
    // cout<<"edges"<<'\n';
    // rng(i,0,11){
    //     for(auto ttt : edges[i]){
    //         cout<<i<<' '<<ttt.fi<<' '<<ttt.se<<'\n';
    //     }
    // }
    int sz=0;
