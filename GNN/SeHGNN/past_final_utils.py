...
def hg_sym_propagate(new_g, tgt_type, num_hops, max_hops, extra_metapath, mode, f_log, debug=False, echo=True):
    for hop in range(1, max_hops):
        reserve_heads = [ele[:hop] for ele in extra_metapath if len(ele) > hop] # 없음
        ...
        for etype in new_g.etypes:
            ...
            if len(stype)==1 or len(dtype)==1: continue # 이게 왜 있지..? label propagation의 필요가 있다.
            f_log.write(f"DEBUG_hg2.2 | stype : {stype} | dtype : {dtype} | type(new_g.nodes[stype]) : {type(new_g.nodes[stype])}\n")

            for k in list(new_g.nodes[stype].data.keys()): # 아마 stype -> dtype으로 propagation
                if len(reduce_year(k)) == hop: # 딱 정해진 길이의 것만 처리를 해주는 것이다.
                    current_dst_name = f'{dtype}{k}' # {dtype}{stype}으로 네이밍
        ...
        for ntype in new_g.ntypes:
            if ntype[0] == tgt_type: continue # 아니 tgt_type하고 같아도 짧으면 reduce해야하는거 아닌가 싶었는데 아니었다. 위에서 len을 ㅔ크하는군
            removes = []
            for k in new_g.nodes[ntype].data.keys():
                ...
                if len(reduce_year(k)) <= hop: # 아니 왜 다 없애..? # hop이라는게 만고불변이 아니라 1 hop값을 없애는 거.
                    # 왜냐면 다음 iteration에서 방금 한 걸 다시해버리면 낭비니까
                    removes.append(k)
            for k in removes:
    ...
    f_log.flush()
    # P2010 ~ P2019 is node itself, not data keys. # P2010은 key가 될 수 있는 것
    ...
    for ntype in new_g.ntypes:
        ...
        for k in new_g.nodes[ntype].data.keys():
            ...
            year_idx=np.squeeze(np.argwhere(paper_year==yy)) # paper_year==yy가 조건문이 아니라 브로드캐스팅된 배열임 (payer_year와 사이즈가 같고)
            # 그걸 idx의 배열로 만들 필요가 있었던듯
    ...
    for ntype in new_g.ntypes:
        if ntype[0]!='P': continue # 마지막에는 정말 P로 모이는 것 제외하고 모두 없애기 가능
        removes=[]
        for k in new_g.nodes[ntype].data.keys():
...
def hg_propagate(new_g, tgt_type, num_hops, max_hops, extra_metapath, mode, f_log, echo=True):
    ...
    for hop in range(1, max_hops):
        ...
        for etype in new_g.etypes:
            ...
            f_log.write(f"DEBUG_hg2 | etype : {etype}\n")
            # A-I A-P F-P I-A P-A P-F P-P # P>P가 있지 않는가?
            f_log.flush()
            stype, direction, dtype = new_g.to_canonical_etype(etype)
...
def load_mag(args, f_log, time_bit):
    ...
    n_classes = int(init_labels.max()) + 1 # 이건 0이 있나? 
    evaluator = get_ogb_evaluator(args.dataset)
    ...
    PP_sym = adjs[2].to_symmetric() #  ('paper', 'cites', 'paper') 임
    ...
    for etype, adj in zip(etypes, adjs):
        ...
        if stype == 'P' and dtype == 'P':
            ...
            # For new feature propagation
            for yy in range(2010,2020):
                if not (os.path.exists(args.root+args.synthesis_mode+'symgraph/'+stype+dtype+str(yy)+'src.npy') and os.path.exists(args.root+'subgraph/'+stype+dtype+str(yy)+'dst.npy')):
                    newsrc=[]
                    newdst=[]
                    for i in range(len(src)):
                        newsrc.append(src[i])
                        newdst.append(dst[i]) # src -> dst로 감에 주의. 
                    ...
                    if(args.synthesis_mode[0]=='o'):
                        newsrc, newdst = np.array(newsrc), np.array(newdst) # 혹시 reversing이 필요한 건 아닌지..?
                    else:
                        newsrc, newdst = np.array(newdst), np.array(newsrc) # 혹시 reversing이 필요한 건 아닌지..?
                    np.save(args.root+args.synthesis_mode+'symgraph/'+stype+dtype+str(yy)+'src',newsrc)
                    np.save(args.root+args.synthesis_mode+'symgraph/'+stype+dtype+str(yy)+'dst',newdst)
                else:
                    newsrc=np.load(args.root+args.synthesis_mode+'symgraph/'+stype+dtype+str(yy)+'src.npy')
                    newdst=np.load(args.root+args.synthesis_mode+'symgraph/'+stype+dtype+str(yy)+'dst.npy')
                ...
                f_log.flush()
                new_edges[('P'+str(yy), 'P'+str(yy)+'-'+'P'+str(yy), 'P'+str(yy))] =(newsrc, newdst) # 방향에 대한 자신이 하나도 없음.. 
                num_nodes['P'+str(yy)]=nums['P']
            num_nodes['P']=nums['P']

            # Still need this part because of label propagation. # 아 label propagation은 P-P가 필요하구나...
            for i in range(len(src)):
            ...
            new_edges[(stype, stype+'>'+dtype, dtype)] = (dst, src) # 엥 이거 왜 dst, src인거지?

            # 이게 toggle의 경우에 label propagation에서 이걸 쓰는 듯 하다. 근데 왜 역방향으로 넣는지는 잘 모르겠다.
            # 사실 지금은 toggle을 잘 안쓰니 상관없다.

            # 애초에 이걸 쓰나? label propagation에서만 쓰일텐데
            ...
            for yy in range(2010,2020): # P가 하나만 있어도 연도별로 다 끊어냄. 정확히는 P의 연도가 yy이하면 되는 조건
                print(f"yy : {yy} | src : {stype} | dst : {dtype}")
                if not (os.path.exists(args.root+'symgraph/'+stype+dtype+str(yy)+'src.npy') and os.path.exists(args.root+'subgraph/'+stype+dtype+str(yy)+'dst.npy')):
                    f_log.write(f"yy : {yy} | src : {stype} | dst : {dtype}\n")
                    f_log.flush()
                    newsrc=[]
                    newdst=[]
                    for i in range(len(src)):
                        if paper_year[src[i]]<=yy: # src -> dst가 맞고 
                            newsrc.append(src[i])
                            newdst.append(dst[i])
            ...
            new_edges[(stype, stype+'-'+dtype, dtype)] = (src, dst) # 그니까 src, dst가 맞는 것 아닌가?
            ...
            for yy in range(2010,2020): # 둘 다 P가 없는데도 연도별로 symgraph를 만듬. (?) 내용은 다 똑같음
    ...
    # Identity에 대한 행렬곱. 이건 label propagation할 때 label leakage를 방어하기 위해 빼줘야 한다.
