...
def hg_year_propagate(new_g, tgt_type, num_hops, max_hops, extra_metapath, mode, f_log, debug=False, echo=True):
...
def load_mag(args, f_log, time_bit):
    ...
    for etype, adj in zip(etypes, adjs):
        ...
        if stype == 'P' and dtype == 'P':
            ...
            for yy in range(2010,2020):
                if not (os.path.exists(args.root+'subgraph/'+stype+dtype+str(yy)+'src.npy') and os.path.exists(args.root+'subgraph/'+stype+dtype+str(yy)+'dst.npy')):
                    newsrc=[]
                    newdst=[]
                    for i in range(len(src)):
                        if paper_year[src[i]]<=yy and paper_year[dst[i]]<=yy:
                            newsrc.append(src[i])
                            newdst.append(dst[i])
                    newsrc, newdst = np.array(newsrc), np.array(newdst)
                    np.save(args.root+'subgraph/'+stype+dtype+str(yy)+'src',newsrc)
                    np.save(args.root+'subgraph/'+stype+dtype+str(yy)+'dst',newdst)
                else:
                    newsrc=np.load(args.root+'subgraph/'+stype+dtype+str(yy)+'src.npy')
                    newdst=np.load(args.root+'subgraph/'+stype+dtype+str(yy)+'dst.npy')
                ...
                f_log.flush()
                new_edges[('P'+str(yy), 'P'+str(yy)+'-'+'P'+str(yy), 'P'+str(yy))] =\
                        (np.concatenate((newsrc, newdst)), np.concatenate((newdst, newsrc)))
            ...
            for yy in range(2010,2020): # 둘 다 P가 없는데도 연도별로 subgraph를 만듬. (?) 내용은 다 똑같음
    ...
    diag_name = f'{args.root}subgraph/{args.dataset}_PFP_diag.pt'
    if not os.path.exists(diag_name):
        PF = FP.t()
        PFP_diag = sparse_tools.spspmm_diag_sym_ABA(PF)
        torch.save(PFP_diag, diag_name)

    diag_name = f'{args.root}subgraph/{args.dataset}_PPP_diag.pt'
    if not os.path.exists(diag_name):
        # PP = PP.to_symmetric()
    ...
    diag_name = f'{args.root}subgraph/{args.dataset}_P<P<P_diag.pt'
    PPP_diag = sparse_tools.spspmm_diag_ABA(PP,PP)
    torch.save(PPP_diag, diag_name)

    diag_name = f'{args.root}subgraph/{args.dataset}_P<PP_diag.pt'
    PPP_diag = sparse_tools.spspmm_diag_ABA(PP,PP_sym)
    torch.save(PPP_diag, diag_name)


    diag_name = f'{args.root}subgraph/{args.dataset}_PAP_diag.pt'
    if not os.path.exists(diag_name):
