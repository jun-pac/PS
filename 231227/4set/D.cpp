...
void Solve(){
    ...
    rng(i,0,q-1){
        ...
        if(a==1){
            cin>>b;
            // sum이 b인 것이 존재하는지
            ll tot=seg.find_seg(1,0,n-1,0,n-1);
            if(tot<b){
                cout<<"NO"<<'\n';
            }
            else{
                int l=0,r=n-1; // [0,l]의 sum이 b보다 크거나 같은 최대의 값
                while(l<r){
                    int mid=(l+r)>>1;
                    if(seg.find_seg(1,0,n-1,0,mid)>=b) r=mid;
                    else l=mid+1;
                } 
                if(seg.find_seg(1,0,n-1,0,l)==b) cout<<"YES"<<'\n';
                else if(seg.find_seg(1,0,n-1,l,n-1)==2*(n-l) && seg.find_seg(1,0,n-1,0,n-1-l)==2*(n-l)) cout<<"NO"<<'\n';
                else cout<<"YES"<<'\n';
            }
        }
        else{
