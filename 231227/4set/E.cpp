...
int datas[N];
int pos[2*N]; // 각 idx에 매칭되는 val의 위치
int ans[N];
set<int> st;
...
void Solve(){
    ...
    seg.build_seg(1,0,2*n-1);
    st.clear();
    gnr(i,n-1,0){
        if(datas[i]<n) seg.update_seg(1,0,2*n-1,datas[i]+n);   
    }
    gnr(i,n-1,0){
        ans[datas[i]%n]=(datas[i]-i)-seg.find_seg(1,0,2*n-1,i,datas[i]);
        seg.update_seg(1,0,2*n-1,datas[i]);
    }
